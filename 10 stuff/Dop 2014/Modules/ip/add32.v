...
`timescale 1 ns / 1 ps
module add32 (DataA, DataB, Result)/* synthesis NGD_DRC_MASK=1 */;
    ...
    FADD2 addsub_0 (.A1(DataA[1]), .A0(DataA[0]), .B1(DataB[1]), .B0(DataB[0]), 
        .CI(scuba_vlo), .COUT1(co0), .COUT0(), .S1(Result[1]), .S0(Result[0]));
    ...
    FADD2 addsub_15 (.A1(DataA[31]), .A0(DataA[30]), .B1(DataB[31]), .B0(DataB[30]), 
        .CI(co14), .COUT1(), .COUT0(), .S1(Result[31]), .S0(Result[30]));
