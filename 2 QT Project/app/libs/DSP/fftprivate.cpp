...
FFTPrivate::FFTPrivate (const long length, DSP::FFT* aPublicAPI):
    d(aPublicAPI)
    ,_length (length)
    ,_nbr_bits (int (floor (log ((float)length) / log (2.0) + 0.5)))
    ,_bit_rev_lut (int (floor (log ((float)length) / log (2.0) + 0.5)))
    ,_trigo_lut (int (floor (log ((float)length) / log (2.0) + 0.5)))
...
void    FFTPrivate::do_fft (flt_t f [], const flt_t x []) const
{
...
void    FFTPrivate::do_ifft (const flt_t f [], flt_t x []) const
{
...
void    FFTPrivate::rescale (flt_t x []) const
{
...
FFTPrivate::BitReversedLUT::BitReversedLUT (const int nbr_bits)
{
    long                                length;
    long                                cnt;
    long                                br_index;
...
FFTPrivate::BitReversedLUT::~BitReversedLUT (void)
{
    delete [] _ptr;
    _ptr = 0;
}
...
FFTPrivate::TrigoLUT::TrigoLUT (const int nbr_bits)
{
    long                total_len;
...
FFTPrivate::TrigoLUT::~TrigoLUT (void)
{
    delete [] _ptr;
    _ptr = 0;
}
